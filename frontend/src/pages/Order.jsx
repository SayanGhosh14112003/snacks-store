...
import { PackageCheck, MapPin, Wallet, Calendar } from "lucide-react";
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      {myOrders?.map((order) => (
        <div
          key={order._id}
          className="border rounded-lg shadow-sm bg-white p-5 hover:shadow-md transition-shadow duration-300"
        >
          {/* Header */}
          ...
          <div className="space-y-3">
            {order.products.map((item) => (
              <div
                key={item._id}
                className="flex flex-col sm:flex-row items-center justify-between border rounded-md p-3 bg-gray-50"
              >
